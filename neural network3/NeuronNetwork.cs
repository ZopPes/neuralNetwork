    ...
    public class NeuronNetwork
    {
        ...
        public double Beckpropagation(double[] exprected,params double[] input)
        {
            ...
            for (int i = 0; i < Layers.Last().Cout; i++)
            {
                ...
                neuron.Learn( exprected[i],Topolodgi.LearningRate);
            }
            ...
            for (int i = Layers.Count - 2; i >= 0; i--)
            {
                ...
                for (int j = 0; j < Layer.Cout; j++)
                {
                    ...
                    for (int k = 0; k < neuron.OutputWeight.Count; k++)
                    {
                        neuron.Learn( Topolodgi.LearningRate);
                    }
                }
            }
            var differens = 0.0;
            ...
            {
                    differens += Math.Pow(neuron.Delta, 2);
            }
            return differens;
        }
